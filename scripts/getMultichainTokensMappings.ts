...
async function main(): Promise<void> {
  for (const blockchainName in blockchains) {
    ...
    const response = await fetch('https://bridgeapi.anyswap.exchange/v4/tokenlistv4/' + blockchains[blockchainName].chainID, {
        method: 'GET',
        headers: {
    ...
    for (const key in jsonData) {
      ...
      for (const chainKey in destChains) {
        ...
        for (const chainPathKey in chainPaths) {
          if (chainPaths[chainPathKey].routerABI === "anySwapOutUnderlying(fromanytoken,toAddress,amount,toChainID)") {
            fetchedTokens.push(
              {
    ...
    fs.writeFileSync('config/multichainTokens.json', JSON.stringify(blockchains, null, 2));
