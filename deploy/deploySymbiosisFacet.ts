...
export default async function (hre: HardhatRuntimeEnvironment) {
  ...
  // Load contract
  const artifact = await deployer.loadArtifact('SymbiosisFacet')

  const rawdata = fs.readFileSync(`config/symbiosis.json`, 'utf-8')
  ...
  constructorArgs = [config.metaRouter, config.gateway]
